...
orientability:
			tangent bundles have d/dx^i as coordinates, implied by x^i coordinates 
			|dx/dy| > J for all overlapping charts means orientable

			ooh ok so configuration space is tangent bundle, and phase space is cotangent bundle


## Differentiable Manifolds

This is the natural setting in which to think about space and time, so has important physical applications.

These are manifolds where the transitions between local isomorphism to $\R^n$ are differentiable. The consequence, once we spell out the details, is that we can talk about a fiber bundle, where the base space is the manifold, and to every point $p$ on the manifold, we associate a vector space, the fiber, $T_p(\mathcal{M})$, where $T_p(\mathcal{M})$ is of the same dimension $d$ as the manifold, and therefore isomorphic to $\mathbb{R}^d$. Conceptually, it is the space of tangent vectors. ($T^*_p\mathcal{M}$ is the cotangent space.)
...
There is a crucial piece of information absent from the definition of a fiber bundle, namely a way to lift a path on the base manifold $B$ to a path on the whole space $E$ (such that the path on $E$ can be pushed down onto $B$ by the bundle projection to recover the original path on $B$). This is known as parallel transport. The natural way to specify this is by a Lie algebra valued 1-form on a bundle.

!!! Note

    Connections, parallel transport, and covariant derivative are all separate notions, but from a physical perspective, cleaving these things apart is not so important.

We call such an object a *connection*; from the physical perspective, it is expedient to view a *connection* as the data which determines parallel transport (although in truth the notions can be useful for separate reasons). 

That is, for an infinitesimal change on the base manifold, we want a Lie algebra element whose exponential describes the way the fiber changes, since that then characterizes how the whole space changes (recalling that a fiber bundle is locally trivial). We can then integrate to obtain parallel transport.

TODO: below

For a **vector** bundle with base space $B$ and fiber $F$, a connection is a function $A : (p : B) \to TM_p \to (F_p \to F_p)$. This is precisely the data you need to define an ODE on $\phi : [0, T] \to F$, with $\gamma : [0,T] \to B$, namely:
...
## Trivializing the cohomology

## Gauge transformations on bundles
...
Then any loop $\gamma$ on $M$ yields a linear map from the start to the end of the lifted path on the fiber, known as the *holonomy* of the loop, which transforms under a gauge transform like: 
...
Suppose we're on a vector bundle. Then

# note

all smooth linear maps $\Gamma(E) \to \Gamma(E) \cong \Gamma(End(E))\cong \Gamma(E \otimes E^*)$

## working stuff out

so in a gauge theory, one includes a field A which is valued in the Lie algebra of some (possibly non-Abelian) group

gauge transformation is map between different local sections
...
A connection (locally on a manifold):
...
for $a(t)$ Abelian. Path order otherwise.
